			...
			<div class="columns mbs-2">
				...
				>
					...
					<div
						...
						class="task-section attachments"
					>
						<Attachments
...
async function attachmentUpload(file: File, onSuccess?: (url: string) => void) {
	const uploaded = await uploadFile(task.value.id, file, onSuccess)
	...
	async (value) => {
		...
		try {
			...
			if (taskIdentifier.value) {
				loaded = await taskService.getByIdentifier(taskIdentifier.value, {expand})
			} else {
				loaded = await taskService.get({id: numericTaskId.value}, {expand})
			}
...
.columns.details {
	...
	&.assignees,
	&:not(.assignees) {
		.column:has(> .detail-title + .datepicker),
		.column:has(> .detail-title + .date-input),
		.column:has(> .detail-title + .multiselect),
		.column:has(> .detail-title + input),
		.column:has(> .detail-title + select),
		.column:has(> .detail-title + .priority-select),
		.column:has(> .detail-title + .percent-done-select),
		.column:has(> .detail-title + .color-picker),
		.column:has(> .detail-title + .reminders),
		.column:has(> .detail-title + .repeat-after) {
			border: 0;
		}
	}
}
