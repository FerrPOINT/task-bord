...
async function loadBuckets() {
	...
	try {
		...
		buckets.value = await service.getAll({projectId: projectId.value, projectViewId: props.viewId})
	} catch (e) {
		console.error('Failed to load buckets:', e)
	}
}

watch(() => props.viewId, loadBuckets)
...
const {
	...
	() => projectId.value === -1
		...
		: ['subtasks', 'comment_count', 'is_unread', 'buckets'],
)
