		...
		<CustomTransition name="fade">
			...
			>
				<BaseButton
					...
					@keydown.up.prevent="() => preSelectPrev(index)"
					@keydown.down.prevent="() => preSelectNext(index)"
...
function setResult(el: any, indexRaw: number | string) {
...
function preSelect(index: number | string) {
...
function preSelectPrev(index: number | string) {
	preSelect(Number(index) - 1)
}
function preSelectNext(index: number | string) {
	preSelect(Number(index) + 1)
}
function create() {
	if (query.value === '') {
