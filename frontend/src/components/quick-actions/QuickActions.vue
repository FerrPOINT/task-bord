...
async function doAction(type: ACTION_TYPE, item: DoAction) {
	switch (type) {
		...
		case ACTION_TYPE.TASK:
			if (isQuickAddMode) {
				const channel = new BroadcastChannel('task-board-task-updates')
				...
				await router.push({
					name: 'task.detail',
					params: {identifierOrId: (item as DoAction<ITask>).identifier || (item as DoAction<ITask>).id},
				})
			}
			closeQuickActions()
			break
...
async function newTask() {
	...
	await router.push({name: 'task.detail', params: {identifierOrId: task.identifier || task.id}})
}
