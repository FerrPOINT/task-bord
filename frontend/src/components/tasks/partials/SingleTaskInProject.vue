			...
			>
				<span class="is-inline-flex is-align-items-center">
					...
					<span
						...
						class="task-identifier tag is-light is-small"
					>
						{{ task.identifier }}
					</span>
				</span>
...
const props = withDefaults(defineProps<{
	...
	allTasks?: ITask[],
	buckets?: any /* eslint-disable-line @typescript-eslint/no-explicit-any */[],
}>(), {
	isArchived: false,
	showProject: false,
	disabled: false,
...
const bucketTitle = computed(() => {
	if (task.value.buckets?.length > 0) {
		return task.value.buckets[0].title
	}
	if (!props.buckets || props.buckets.length === 0) {
		return ''
	}
	const bucket = props.buckets.find(b => b.id === task.value.bucketId)
	return bucket?.title || ''
})

const taskDetailRoute = computed(() => ({
	name: 'task.detail',
	params: {identifierOrId: task.value.identifier || task.value.id},
