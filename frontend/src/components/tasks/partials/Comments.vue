<template>
	<div
		v-if="enabled"
		ref="commentsRef"
		class="task-section comments-container"
	>
		<h3
		...
		<div class="comments">
			...
			<div
				v-if="canWrite"
				class="media comment d-print-none new-comment"
			>
				<div class="media-content">
					<div class="form">
						<CustomTransition name="fade">
...
async function editCommentWithDelay() {
	if (commentEdit.comment === comments.value.find(c => c.id === commentEdit.id)?.comment) {
...
async function editComment() {
	...
	try {
		const updated = await taskCommentService.update(commentEdit)
		const index = comments.value.findIndex(c => c.id === updated.id)
		if (index !== -1) {
			comments.value[index] = updated
		}
		saved.value = updated.id
		setTimeout(() => {
			saved.value = null
		}, 2000)
		clearEditorDraft(`task-comment-${updated.id}`)
	} finally {
		saving.value = null
	}
}
...
.new-comment {
	...
	:deep(.tiptap__editor-is-edit-enabled) {
		background: var(--scheme-main-bis);
	}
}
...
.new-comment :deep(.tiptap__editor-is-edit-enabled) {
