...
export const checkAndSetApiUrl = (pUrl: string | undefined | null): Promise<string> => {
	...
	// Normalize relative paths to absolute URLs using the current origin.
	// This also rewrites stale version suffixes like /api/v2 to /api/v1.
	if (url.startsWith('/')) {
	...
	// Always target /api/v1, regardless of any stale suffix stored locally.
	urlToCheck.pathname = API_PATH_SUFFIX
