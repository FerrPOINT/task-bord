...
func TestTask_ReadOne(t *testing.T) {
	...
	t.Run("created by a non-existing user", func(t *testing.T) {
		db.LoadAndAssertFixtures(t)
		s := db.NewSession()
		defer s.Close()
		...
		assert.Equal(t, int64(1), task.CreatedByID)
...
func TestGetTaskByIdentifier(t *testing.T) {
	t.Run("existing task", func(t *testing.T) {
		...
		task, err := GetTaskByIdentifier(s, "TEST1-1")
		require.NoError(t, err)
		assert.Equal(t, int64(1), task.ID)
		assert.Equal(t, "task #1", task.Title)
	})
	...
	t.Run("case insensitive", func(t *testing.T) {
		...
		task, err := GetTaskByIdentifier(s, "test1-3")
		require.NoError(t, err)
		assert.Equal(t, int64(3), task.ID)
	})

	t.Run("nonexistent identifier", func(t *testing.T) {
		db.LoadAndAssertFixtures(t)
		s := db.NewSession()
		defer s.Close()

		_, err := GetTaskByIdentifier(s, "UNKNOWN-1")
		require.Error(t, err)
		assert.True(t, IsErrProjectDoesNotExist(err))
	})
	...
	t.Run("nonexistent index", func(t *testing.T) {
		...
		_, err := GetTaskByIdentifier(s, "TEST1-99999")
		require.Error(t, err)
		assert.True(t, IsErrTaskDoesNotExist(err))
	})
	...
	t.Run("invalid format", func(t *testing.T) {
		...
		_, err := GetTaskByIdentifier(s, "123-1")
		require.Error(t, err)
		assert.True(t, IsErrTaskDoesNotExist(err))

		_, err = GetTaskByIdentifier(s, "not-an-identifier")
		require.Error(t, err)
		assert.True(t, IsErrTaskDoesNotExist(err))
	})
}

func TestTaskSetIdentifier(t *testing.T) {
	t.Run("with project identifier", func(t *testing.T) {
		task := Task{Index: 42}
		task.setIdentifier(&Project{Identifier: "PROJ"})
		assert.Equal(t, "PROJ-42", task.Identifier)
	})

	t.Run("without project identifier", func(t *testing.T) {
		task := Task{Index: 42}
		task.setIdentifier(&Project{Identifier: ""})
		assert.Equal(t, "#42", task.Identifier)
	})

	t.Run("nil project", func(t *testing.T) {
		task := Task{Index: 42}
		task.setIdentifier(nil)
		assert.Equal(t, "#42", task.Identifier)
	})
}
