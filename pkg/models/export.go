...
func ExportUserData(s *xorm.Session, u *user.User) (err error) {
	dumpFile, err := os.CreateTemp("", "taskboard-export-*.zip")
	if err != nil {
	...
	// If we reuse the same file again, saving it as a file in taskboard will save it as a file with 0 bytes in size.
