...
func TestProject_CreateOrUpdate(t *testing.T) {
	...
	t.Run("create", func(t *testing.T) {
		...
		t.Run("kanban view creates default buckets", func(t *testing.T) {
			db.LoadAndAssertFixtures(t)
			s := db.NewSession()
			defer s.Close()
			project := Project{
			...
			// Check that the expected number of buckets were created
			var bucketCount int64
			bucketCount, err = s.Where("project_view_id = ?", kanbanView.ID).Count(&Bucket{})
			require.NoError(t, err)
			assert.Equal(t, int64(6), bucketCount, "Should have created six buckets")

			// Check that the buckets are named correctly
			var buckets []*Bucket
			err = s.Where("project_view_id = ?", kanbanView.ID).OrderBy("position ASC").Find(&buckets)
			require.NoError(t, err)
			require.Len(t, buckets, 6, "Should have six buckets")
			assert.Equal(t, "СПИСОК ЗАДАЧ", buckets[0].Title)
			assert.Equal(t, "ВЫБРАНО ДЛЯ РАЗРАБОТКИ", buckets[1].Title)
			assert.Equal(t, "В РАБОТЕ", buckets[2].Title)
			assert.Equal(t, "РЕВЬЮ", buckets[3].Title)
			assert.Equal(t, "ТЕСТИРОВАНИЕ", buckets[4].Title)
			assert.Equal(t, "ВЫПОЛНЕНО", buckets[5].Title)

			// Check that first bucket is the default bucket
			assert.Equal(t, buckets[0].ID, kanbanView.DefaultBucketID, "СПИСОК ЗАДАЧ should be the default bucket")

			// Check that last bucket is the done bucket
			assert.Equal(t, buckets[5].ID, kanbanView.DoneBucketID, "ВЫПОЛНЕНО should be the done bucket")
		})
		...
		t.Run("invalid identifier format", func(t *testing.T) {
			...
			err := project.Create(s, usr)
			require.Error(t, err)
			assert.True(t, IsErrInvalidProjectIdentifier(err))
		})
		t.Run("identifier normalized to uppercase", func(t *testing.T) {
			db.LoadAndAssertFixtures(t)
			s := db.NewSession()
			defer s.Close()
			project := Project{
