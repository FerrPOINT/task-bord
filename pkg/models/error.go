...
// GetCode returns taskboard's numeric domain error code. v2's translateDomainError
...
// ErrInvalidProjectIdentifier represents an error where the project identifier has an invalid format.
type ErrInvalidProjectIdentifier struct {
	Identifier string
}

// IsErrInvalidProjectIdentifier checks if an error is an ErrInvalidProjectIdentifier.
func IsErrInvalidProjectIdentifier(err error) bool {
	_, ok := err.(ErrInvalidProjectIdentifier)
	return ok
}

func (err ErrInvalidProjectIdentifier) Error() string {
	return fmt.Sprintf("Project identifier '%s' is invalid.", err.Identifier)
}

// ErrCodeInvalidProjectIdentifier holds the unique world-error code of this error
const ErrCodeInvalidProjectIdentifier = 3007

// HTTPError holds the http error description
func (err ErrInvalidProjectIdentifier) HTTPError() web.HTTPError {
	return web.HTTPError{
		HTTPCode: http.StatusBadRequest,
		Code:     ErrCodeInvalidProjectIdentifier,
		Message:  "Project identifier must be 1-10 characters long, start with a letter and contain only letters, digits and underscores.",
	}
}
// ErrProjectIdentifierRequired represents an error where the project identifier is missing.
type ErrProjectIdentifierRequired struct{}

// IsErrProjectIdentifierRequired checks if an error is an ErrProjectIdentifierRequired.
func IsErrProjectIdentifierRequired(err error) bool {
	_, ok := err.(ErrProjectIdentifierRequired)
	return ok
}

func (err ErrProjectIdentifierRequired) Error() string {
	return "Project identifier is required."
}

// ErrCodeProjectIdentifierRequired holds the unique world-error code of this error
const ErrCodeProjectIdentifierRequired = 3010

// HTTPError holds the http error description
func (err ErrProjectIdentifierRequired) HTTPError() web.HTTPError {
	return web.HTTPError{
		HTTPCode: http.StatusBadRequest,
		Code:     ErrCodeProjectIdentifierRequired,
