...
type Task struct {
	...
	// It can be a numeric id or a textual identifier like "PROJ-123".
...
// @Param filter query string false "The filter query to match tasks by. Check out https://taskboard.io/docs/filters for a full explanation of the feature."
...
// taskIdentifierRe parses a textual task identifier (e.g. "PROJ-123") into a
// project identifier and a per-project task index.
var taskIdentifierRe = regexp.MustCompile(`^([A-Z][A-Z0-9_]{0,9})-(\d+)$`)

// GetTaskByIdentifier looks up a task by its textual identifier (e.g. "PROJ-123").
// It returns ErrTaskDoesNotExist if the identifier format is invalid or no task matches.
func GetTaskByIdentifier(s *xorm.Session, identifier string) (task Task, err error) {
	matches := taskIdentifierRe.FindStringSubmatch(strings.ToUpper(identifier))
	if len(matches) != 3 {
		return Task{}, ErrTaskDoesNotExist{}
	}

	projectIdentifier := matches[1]
	taskIndex, err := strconv.ParseInt(matches[2], 10, 64)
	if err != nil {
		return Task{}, ErrTaskDoesNotExist{}
	}

	project, err := GetProjectByIdentifier(s, projectIdentifier)
	if err != nil {
		return Task{}, err
	}

	return GetTaskByProjectAndIndex(s, project.ID, taskIndex)
}

// GetProjectByIdentifier looks up a project by its short textual identifier.
func GetProjectByIdentifier(s *xorm.Session, identifier string) (project Project, err error) {
	has, err := s.
		Where("identifier = ?", strings.ToUpper(identifier)).
		Get(&project)
	if err != nil {
		return Project{}, err
	}
...
// ResolveIdentifier populates t.ID and t.ProjectID/Index from a textual task
// identifier when t.ID is not already set. It can be called before ReadOne.
func (t *Task) ResolveIdentifier(s *xorm.Session) error {
	if t.ID != 0 {
		return nil
	}
	resolved, err := GetTaskByIdentifier(s, t.Identifier)
	if err != nil {
		return err
	}
	*t = resolved
...
func (t *Task) ResolveProjectTaskParam(s *xorm.Session) error {
	...
	resolved, err := GetTaskByIdentifier(s, t.ProjectTaskParam)
	if err != nil {
		return err
	}
	t.ID = resolved.ID
...
func createTask(s *xorm.Session, t *Task, a web.Auth, updateAssignees bool, setBucket bool) (err error) {
	...
	createdBy, err := GetUserByAuth(s, a)
	if err != nil {
		return err
	}
...
func (t *Task) updateSingleTask(s *xorm.Session, a web.Auth, fields []string) (err error) {
	...
	if err = t.ResolveProjectTaskParam(s); err != nil {
		return
	}
...
// addTimeEntriesCountToTasks is a legacy no-op since time tracking was removed.
func addTimeEntriesCountToTasks(_ *xorm.Session, _ web.Auth, _ []int64, _ map[int64]*Task) error {
	return nil
}
