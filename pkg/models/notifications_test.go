...
func TestGetThreadID(t *testing.T) {
	...
	t.Run("simple domain without port", func(t *testing.T) {
		config.ServicePublicURL.Set("https://taskboard.example.com")
		threadID := getThreadID(456)
		assert.Equal(t, "<task-456@taskboard.example.com>", threadID)
	})

	t.Run("domain with standard HTTPS port", func(t *testing.T) {
		config.ServicePublicURL.Set("https://taskboard.example.com:443")
		threadID := getThreadID(789)
		// Should strip port to create valid RFC 5322 domain
		assert.Equal(t, "<task-789@taskboard.example.com>", threadID)
	})
	...
	t.Run("domain with port 3456", func(t *testing.T) {
		config.ServicePublicURL.Set("http://taskboard.local:3456")
		threadID := getThreadID(111)
		// Should strip port to create valid RFC 5322 domain
		assert.Equal(t, "<task-111@taskboard.local>", threadID)
	})
	...
	t.Run("URL with path", func(t *testing.T) {
		config.ServicePublicURL.Set("https://example.com:9000/taskboard")
...
func TestUndoneTasksOverdueNotification_TitleIsMarkdownEscaped(t *testing.T) {
	...
	assert.Contains(t, opts.HTMLMessage, "https://taskboard.example.com/tasks/42",
		"legitimate task link must still render")
	// Exactly one anchor to the task — the injection must not create a
	// second one. taskboard templates also render the Action link, but this
	// notification has no Action for the individual task.
	assert.Equal(t, 1, strings.Count(opts.HTMLMessage, `<a href="https://taskboard.example.com/tasks/42`),
