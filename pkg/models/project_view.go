...
// Schema lets Huma (/api/v1) reflect this type as a string enum. The custom
...
// Schema lets Huma (/api/v1) reflect this type as a string enum; see the note
...
type ProjectViewBucketConfiguration struct {
	...
	Filter *TaskCollection `json:"filter" doc:"The filter query that decides which tasks land in this bucket. See https://taskboard.io/docs/filters."`
}
...
type ProjectView struct {
	...
	// The filter query to match tasks by. Check out https://taskboard.io/docs/filters for a full explanation.
	Filter *TaskCollection `xorm:"json null default null" query:"filter" json:"filter" doc:"The filter query used to match tasks shown in this view. See https://taskboard.io/docs/filters."`
...
func createProjectView(s *xorm.Session, p *ProjectView, a web.Auth, createBacklogBucket bool, addExistingTasksToView bool) (err error) {
	...
	if p.ViewKind == ProjectViewKindKanban && createBacklogBucket && p.BucketConfigurationMode == BucketConfigurationModeManual {
		...
		var buckets []*Bucket
		for i, title := range bucketTitles {
			bucket := &Bucket{
				ProjectViewID: p.ID,
				Title:         title,
				Position:      float64((i + 1) * 100),
			}
			err = bucket.Create(s, a)
			if err != nil {
				return
			}
		...
		// Set the first bucket as default and the last one as done bucket
		p.DefaultBucketID = buckets[0].ID
		p.DoneBucketID = buckets[len(buckets)-1].ID
		_, err = s.ID(p.ID).Cols("default_bucket_id", "done_bucket_id").Update(p)
		if err != nil {
			return
		}

		// Move all tasks into the new default bucket when the project already has tasks
		if addExistingTasksToView {
			err = addTasksToView(s, a, p, buckets[0])
			if err != nil {
				return
			}
		}
