...
import (
	...
	"github.com/FerrPOINT/task-bord/pkg/routes"
...
func setupAutoTLS(server *http.Server) {
	...
	if config.ServiceInterface.GetString() != ":443" {
		log.Warningf("taskboard's interface is set to %s, with tls it is recommended to set this to :443", config.ServiceInterface.GetString())
	}
...
var webCmd = &cobra.Command{
	...
	Run: func(_ *cobra.Command, _ []string) {
		...
		log.Infof("taskboard version %s", version.Version)
