...
// registeredTables holds all table beans registered by taskboard packages.
var registeredTables []interface{}

// RegisterTables registers table beans so that Dump and WipeEverything
// only operate on known taskboard tables.
func RegisterTables(tables []interface{}) {
	registeredTables = append(registeredTables, tables...)
}

// RegisteredTableNames returns the table names of all registered taskboard tables.
...
func CreateDBEngine() (engine *xorm.Engine, err error) {
	...
	engine.SetTZLocation(config.GetTimeZone()) // taskboard's timezone
...
func getUserDataDir() (string, error) {
	...
	switch runtime.GOOS {
	case "windows":
		// On Windows, use %LOCALAPPDATA%\taskboard
		...
		dataDir = filepath.Join(localAppData, "taskboard")
	case "darwin":
		// On macOS, use ~/Library/Application Support/taskboard
		home, err := os.UserHomeDir()
		if err != nil {
			return "", err
		}
		dataDir = filepath.Join(home, "Library", "Application Support", "taskboard")
	default:
		// On Linux and other Unix-like systems, use XDG_DATA_HOME or ~/.local/share/taskboard
		xdgDataHome := os.Getenv("XDG_DATA_HOME")
		if xdgDataHome != "" {
			dataDir = filepath.Join(xdgDataHome, "taskboard")
		} else {
			home, err := os.UserHomeDir()
			if err != nil {
				return "", err
			}
			dataDir = filepath.Join(home, ".local", "share", "taskboard")
		}
	}
...
// WipeEverything wipes all taskboard tables and their data. Use with caution...
