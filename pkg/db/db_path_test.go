	...
	tests := []struct {
	...
	}{
		{
			...
			cfg: DatabasePathConfig{
				...
				RootPath:       "/opt/taskboard",
				ExecutablePath: "/opt/taskboard",
			},
			getUserDataDir: mockGetUserDataDir("/home/user/.local/share/taskboard"),
			expected:       "memory",
		},

		{
			...
			cfg: DatabasePathConfig{
				ConfiguredPath: "/var/lib/taskboard/taskboard.db",
				RootPath:       "/opt/taskboard",
				ExecutablePath: "/opt/taskboard",
			},
			getUserDataDir: mockGetUserDataDir("/home/user/.local/share/taskboard"),
			expected:       "/var/lib/taskboard/taskboard.db",
		},
		{
...

		{
			...
			cfg: DatabasePathConfig{
				ConfiguredPath: "taskboard.db",
				RootPath:       "/var/lib/taskboard",
				ExecutablePath: "/opt/taskboard",
			},
			getUserDataDir: mockGetUserDataDir("/home/user/.local/share/taskboard"),
			expected:       "/var/lib/taskboard/taskboard.db",
		},
		{
			...
			cfg: DatabasePathConfig{
				ConfiguredPath: "data/taskboard.db",
				RootPath:       "/var/lib/taskboard",
				ExecutablePath: "/opt/taskboard",
			},
			getUserDataDir: mockGetUserDataDir("/home/user/.local/share/taskboard"),
			expected:       "/var/lib/taskboard/data/taskboard.db",
		},

		{
			...
			getUserDataDir: mockGetUserDataDir("/home/user/.local/share/taskboard"),
			expected:       "/home/user/.local/share/taskboard/taskboard.db",
		},
...

		{
			...
			expected:       "/opt/taskboard/taskboard.db",
		},
		...
		},
		{
			...
			getUserDataDir: mockGetUserDataDir("/home/user/.local/share/taskboard"),
			expected:       "/home/user/.local/share/taskboard",
		},
		{
			name: "path with dots normalized",
			cfg: DatabasePathConfig{
				ConfiguredPath: "/var/lib/taskboard/../taskboard/./db.db",
				RootPath:       "/opt/taskboard",
				ExecutablePath: "/opt/taskboard",
			},
			getUserDataDir: mockGetUserDataDir("/home/user/.local/share/taskboard"),
			expected:       "/var/lib/taskboard/db.db",
		},
	}
...
func Test_resolveDatabasePath_Integration(t *testing.T) {
	t.Run("with explicitly configured rootpath", func(t *testing.T) {
		...
		expected := filepath.Join("/custom/path", "taskboard.db")
	...
	t.Run("with default rootpath uses user data directory", func(t *testing.T) {
		...
		assert.NotEqual(t, filepath.Join(execDir, "taskboard.db"), result)
		assert.Contains(t, result, "taskboard.db")

		switch runtime.GOOS {
		case "windows":
			assert.Contains(t, result, "taskboard")
		case "darwin":
	...
	t.Run("with subdirectory path", func(t *testing.T) {
		...
		expected := filepath.Join("/custom/path", "data", "taskboard.db")
		assert.Equal(t, expected, result)
	})
}
...
func Test_resolveDatabasePath_Windows(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			cfg: DatabasePathConfig{
				ConfiguredPath: "C:\\ProgramData\\taskboard\\taskboard.db",
				RootPath:       "C:\\Program Files\\taskboard",
				ExecutablePath: "C:\\Program Files\\taskboard",
			},
			getUserDataDir: mockGetUserDataDir("C:\\Users\\test\\AppData\\Local\\taskboard"),
			expected:       "C:\\ProgramData\\taskboard\\taskboard.db",
		},
		{
			name: "windows relative path with explicit rootpath",
			cfg: DatabasePathConfig{
				ConfiguredPath: "taskboard.db",
				RootPath:       "C:\\ProgramData\\taskboard",
				ExecutablePath: "C:\\Program Files\\taskboard",
			},
			getUserDataDir: mockGetUserDataDir("C:\\Users\\test\\AppData\\Local\\taskboard"),
			expected:       "C:\\ProgramData\\taskboard\\taskboard.db",
...
func TestGetUserDataDir(t *testing.T) {
	...
	case "windows":
		dataDir := test()
		assert.Contains(t, dataDir, "taskboard")
	case "darwin":
		dataDir := test()
	...
	default:
		...
		t.Run("with XDG_DATA_HOME", func(t *testing.T) {
			...
			dataDir := test()
			assert.Contains(t, dataDir, filepath.Join("/tmp", "taskboard"))
		})
...
func TestIsSystemDirectory(t *testing.T) {
	tests := []struct {
		...
		// Windows system directories
		{
			...
			path:     "C:\\Windows\\System32\\taskboard.db",
			expected: runtime.GOOS == "windows",
		},
		{
			name:     "Windows SysWOW64",
			path:     "C:\\Windows\\SysWOW64\\taskboard.db",
			expected: runtime.GOOS == "windows",
		},
		{
			name:     "Windows root",
			path:     "C:\\Windows\\taskboard.db",
			expected: runtime.GOOS == "windows",
		},
		{
		...
		{
			name:     "/bin",
			path:     "/bin/taskboard.db",
			expected: runtime.GOOS != "windows",
		},
		{
			name:     "/sbin",
			path:     "/sbin/taskboard.db",
			expected: runtime.GOOS != "windows",
		},
		{
			name:     "/usr/bin",
			path:     "/usr/bin/taskboard.db",
			expected: runtime.GOOS != "windows",
		},
		{
			name:     "/etc",
			path:     "/etc/taskboard.db",
			expected: runtime.GOOS != "windows",
		},
		// Non-system directories
		{
			name:     "user home directory (Unix)",
			path:     "/home/user/taskboard.db",
			expected: false,
		},
		{
			name:     "user profile directory (Windows)",
			path:     "C:\\Users\\user\\taskboard.db",
			expected: false,
		},
		{
			name:     "custom directory",
			path:     "/opt/taskboard/taskboard.db",
			expected: false,
		},
		{
			name:     "relative path",
...
func TestIsSystemDirectory_EdgeCases(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Run("false positives - paths containing 'windows' but not system directories", func(t *testing.T) {
			tests := []struct {
			...
			}{
				{
					...
					path: "C:\\myapp\\windows\\data\\taskboard.db",
				},
				{
					name: "windows directory on non-C drive",
					path: "D:\\windows\\taskboard.db",
				},
				{
					name: "user directory named windows",
					path: "C:\\Users\\windows\\taskboard.db",
				},
			}
		...
		t.Run("safe Windows subdirectories", func(t *testing.T) {
			assert.False(t, isSystemDirectory("C:\\Windows\\Temp\\taskboard.db"))
		})
		...
		t.Run("false positives - paths containing system dir names", func(t *testing.T) {
			tests := []struct {
			...
			}{
				{
					...
					path: "/home/bin/taskboard.db",
				},
				{
					name: "/opt/sbin not same as /sbin",
					path: "/opt/sbin/taskboard.db",
				},
				{
					name: "/usr/local/bin is safe",
					path: "/usr/local/bin/taskboard.db",
				},
				{
					name: "/binaries not same as /bin",
					path: "/binaries/taskboard.db",
				},
			}
