...
func TestResolvePath(t *testing.T) {
	...
	ServiceRootpath.Set("/var/lib/taskboard")
	...
	tests := []struct {
	...
	}{
		{
			...
			input:    "/etc/taskboard/config.yml",
			expected: "/etc/taskboard/config.yml",
		},
		{
			name:     "relative path joined with rootpath",
			input:    "files",
			expected: "/var/lib/taskboard/files",
		},
		{
			name:     "relative subdir path joined with rootpath",
			input:    "data/taskboard.db",
