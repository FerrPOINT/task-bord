...
// GetTimeZone returns the time zone configured for taskboard
...
// getRootpathLocation determines the default root path for taskboard data.
...
func getRootpathLocation() string {
	...
	if exeLocation, err := exec.LookPath("taskboard" + exeSuffix); err == nil {
...
func InitDefaultConfig() {
	...
	DatabaseUser.setDefault("taskboard")
	DatabasePassword.setDefault("")
	DatabaseDatabase.setDefault("taskboard")
	DatabasePath.setDefault(ResolvePath("taskboard.db"))
	...
	MailerFromEmail.setDefault("mail@taskboard")
...
func setConfigFromEnv() error {
	...
	for _, envKeyValue := range envKeys {
		...
		if strings.HasPrefix(key, "TASKBOARD_") {
			formattedKey := strings.ToLower(strings.TrimPrefix(key, "TASKBOARD_"))
...
func InitConfig() {
	...
	if err != nil {
		log.Debugf("No home directory found, not using config from ~/.config/taskboard/. Error was: %s\n", err.Error())
	} else {
		viper.AddConfigPath(path.Join(homeDir, ".config", "taskboard"))
	}
