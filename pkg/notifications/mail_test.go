...
`, mailopts.Message)
	...
	t.Run("with action", func(t *testing.T) {
		mail := NewMail().
			...
			Greeting("Hi there,").
			Line("This is a line").
			Line("This **line** contains [a link](https://taskboard.io)").
			Line("And another one").
...
This **line** contains [a link](https://taskboard.io)
...
`, mailopts.Message)
		...
		assert.Contains(t, mailopts.HTMLMessage, `<a href="https://taskboard.io" rel="nofollow">a link</a>`)
...
`, mailopts.Message)
	...
	t.Run("with thread ID", func(t *testing.T) {
		mail := NewMail().
			...
			Greeting("Hi there,").
			Line("This is a line").
			ThreadID("<task-123@taskboard>")

		mailopts, err := RenderMail(mail, "en")
		require.NoError(t, err)
		assert.Equal(t, mail.from, mailopts.From)
		assert.Equal(t, mail.to, mailopts.To)
		assert.Equal(t, "<task-123@taskboard>", mailopts.ThreadID)
	})
...
func TestConversationalMail(t *testing.T) {
	...
	t.Run("Conversational template selection", func(t *testing.T) {
		...
		assert.NotContains(t, mailopts.HTMLMessage, "taskboard")
	...
	t.Run("Formal template still works", func(t *testing.T) {
		...
		assert.Contains(t, mailopts.HTMLMessage, "taskboard")
