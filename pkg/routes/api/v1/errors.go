...
// translateDomainError maps a taskboard domain error (web.HTTPErrorProcessor)
...
// taskBoardErrorModel extends Huma's RFC 9457 body with taskboard's numeric
...
type taskBoardErrorModel struct {
	huma.ErrorModel
	Code int `json:"code,omitempty" readOnly:"true" doc:"taskboard numeric error code; see https://taskboard.io/docs/errors/"`
}

func init() {
	// Replace Huma's default error constructor so both the generated
	// OpenAPI schema and runtime responses use taskBoardErrorModel. Huma
	...
	huma.NewError = func(status int, msg string, errs ...error) huma.StatusError {
		...
		return &taskBoardErrorModel{ErrorModel: huma.ErrorModel{
			Status: status,
	...
	// response itself — bypasses taskboard's CreateHTTPErrorHandler, which for
